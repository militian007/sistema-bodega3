...
import type { CuentaPagar, Proveedor } from '../types/db';
import { fmtUSD, fmtBs, todayISO } from '../lib/format';
import Modal from '../components/Modal';
import { Wallet, Search, DollarSign, AlertCircle } from 'lucide-react';
...
export default function CuentasPagar() {
  ...
  const [openPago, setOpenPago] = useState<CuentaPagar | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    const [cxpR, provR, tasaR] = await Promise.all([
      supabase.from('cuentas_pagar').select('*').order('fecha_emision', { ascending: false }),
      supabase.from('proveedores').select('*'),
      supabase.from('tasa_bcv').select('tasa').eq('fecha', todayISO()).maybeSingle(),
    ]);
    setItems((cxpR.data as CuentaPagar[]) ?? []);
    setProveedores((provR.data as Proveedor[]) ?? []);
  ...
  const filtered = pendientes.filter((i) => {
    const p = proveedores.find((x) => x.id === i.proveedor_id);
    const s = search.toLowerCase();
    return !s || (p?.nombre.toLowerCase().includes(s)) || (i.descripcion ?? '').toLowerCase().includes(s);
  });
  ...
  const pagar = async () => {
    ...
    await supabase.from('cuentas_pagar_pagos').insert({
      cuenta_pagar_id: openPago.id,
      monto_usd: m,
      monto_bs: m * tasa,
      tasa_bcv: tasa,
    ...
    await supabase
      .from('cuentas_pagar')
      .update({ saldo_usd: nuevoSaldoUSD, saldo_bs: nuevoSaldoBS, estado: nuevoEstado })
      .eq('id', openPago.id);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-display text-2xl text-gray-100 tracking-wider">CUENTAS POR PAGAR</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="neon-card p-4 border border-neon-magenta/40">
          <p className="text-[10px] font-mono uppercase text-gray-400">Total por pagar</p>
                  ...
                  <tr key={c.id}>
                    ...
                    <td>
                      <button
                        ...
                        className="btn-magenta !py-1 !px-2 text-xs"
                      >
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button onClick={pagar} disabled={saving} className="btn-magenta">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  <p className="text-gray-200">
                    {proveedores.find((p) => p.id === openPago.proveedor_id)?.nombre}
                  </p>
                </div>
                <div>
